...
void on_connect_cb(uv_stream_t *server, int status) {
    if( status ) return;
    uv_tls_t *sclient = malloc(sizeof(uv_tls_t)); //freed on uv_close callback
    uv_tcp_t *handle = calloc(1, sizeof(uv_tcp_t)); //freed on uv_close callback
    uv_tcp_init(server->loop, handle);
    if( uv_tls_init(server->loop, (evt_ctx_t*)server->data, sclient, handle) < 0 ) {
        free(sclient);
        free(handle);
        return;
    }
